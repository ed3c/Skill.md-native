...
class AndroidDeviceConstraint(_StrictModel):
    ...
    def validate_constraint(self) -> "AndroidDeviceConstraint":
        ...
        if self.min_api_level is not None and self.max_api_level is not None and self.min_api_level > self.max_api_level:
...
class AndroidContract(_StrictModel):
    ...
    def validate_contract(self) -> "AndroidContract":
        ...
        ui_names = [action.name for action in self.actions if isinstance(action, CaptureUiHierarchyAction)]
        if len(ui_names) != len(set(ui_names)):
            raise ValueError("UI hierarchy artifact names must be unique")
        screenshot_names = [action.name for action in self.actions if isinstance(action, CaptureScreenshotAction)]
...
def _unsafe_relative_path(value: str) -> bool:
    ...
    return not value or path.is_absolute() or value.startswith("~") or any(part in {"", ".", ".."} for part in path.parts)
