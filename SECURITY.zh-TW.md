...
Skill.md-native 目前屬於 pre-1.0 Research 或 Alpha software。安全修正套用於最新 `main`，以及 Maintainer 明確發布時的最新 Release。除非 Maintainer 另行聲明，舊 Commit、Fork、Experimental branch、Fixture 與 Archived evidence 不在支援範圍內。
...
Security report 包含 Supply-chain identity confusion、Archive extraction 或 Path traversal、Runtime escape、Evaluator-authority forgery、Evidence／Verdict substitution、Security-gate compensation、Attestation／Key-policy bypass、Transparency receipt forgery、Unsafe provider egress，以及會隱藏 Failed security state 的 Cross-run compatibility pooling。
