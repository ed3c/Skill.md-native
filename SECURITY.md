...
Skill.md-native is pre-1.0 research or alpha software. Security fixes are applied to the latest `main` branch and, when explicitly published, the latest release. Older commits, forks, experimental branches, fixtures, and archived evidence are not supported unless a maintainer states otherwise.
...
```text
https://github.com/ed3c/Skill.md-native/security/advisories/new
```
...
Security reports include supply-chain identity confusion, archive extraction or path traversal, runtime escape, evaluator-authority forgery, evidence or verdict substitution, security-gate compensation, attestation/key-policy bypass, transparency receipt forgery, unsafe provider egress, and cross-run compatibility pooling that hides a failed security state.
